...
static double **malloc_2d(int R, int C){
	double **ptr = malloc(R*(sizeof(double *)+C*sizeof(double)));
	for(int i=0; i<R; i++)
		ptr[i] = (double *)(ptr+R) + i*C;
	return ptr;
}

// dynamically allocate a 3D array
static double ***malloc_3d(int H, int R, int C){
	double ***ptr = malloc(H*sizeof(double **));
	for(int h=0; h<H; h++)
		ptr[h] = malloc_2d(R, C);
	return ptr;
}
...
// free a 3d array
static void free_3d(double ***ptr, int H){
	for(int h=0; h<H; h++)
		free(ptr[h]);
	free(ptr);
}

// get a line of observations
static int get_obs(FILE *fp, int* obs){
	char buf[MAX_LINE];
